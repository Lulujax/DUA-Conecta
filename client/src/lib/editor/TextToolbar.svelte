<script lang="ts">
    ...
    // No podemos desestructurar `selectedElement` porque es un $derived y necesita
    // ser accedido como `store.selectedElement` para mantener la reactividad.
    // Las funciones, sin embargo, se pueden desestructurar.
    const { 
        availableFonts,
        updateElement,
        toggleStyle,
    ...
    // Hacemos una variable local para evitar el "store.selectedElement" repetitivo
    let selectedElement: any;
    $inspect(store.selectedElement); // Esto asegura que reaccionemos a los cambios
...
<div class="text-toolbar" role="toolbar" aria-label="Herramientas de texto" onclick={stopToolbarClick} onmousedown={stopToolbarClick}>
    <select class="toolbar-select" value={selectedElement.fontFamily} onchange={(e) => updateElement(selectedElement.id, { fontFamily: e.currentTarget.value }, true)}>
    ...
    <label class="icon-button color-label" title="Color de texto">
        ...
        <input type="color" class="color-picker-hidden" value={selectedElement.color} oninput={(e) => updateElement(selectedElement.id, { color: e.currentTarget.value }, true)} />
