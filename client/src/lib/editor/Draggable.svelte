<script lang="ts">
	...
	function onDragStart(e: MouseEvent) {
		...
		const startX = e.clientX; const startY = e.clientY; const startElX = element.x; const startElY = element.y;
		...
		function onDragMove(e: MouseEvent) {
			...
			let snappedX = null; let snappedY = null; let showVerticalLine: number | null = null; let showHorizontalLine: number | null = null;
			const draggingCenterH = currentX + element.width / 2; const draggingCenterV = currentY + element.height / 2; const draggingLeft = currentX; const draggingRight = currentX + element.width; const draggingTop = currentY; const draggingBottom = currentY + element.height;
			for (const other of allElements) { if (other.id === element.id) continue; const otherCenterH = other.x + other.width / 2; const otherLeft = other.x; const otherRight = other.x + other.width; if (Math.abs(draggingCenterH - otherCenterH) < snapThreshold) { snappedX = otherCenterH - element.width / 2; showVerticalLine = otherCenterH; break; } if (Math.abs(draggingLeft - otherLeft) < snapThreshold) { snappedX = otherLeft; showVerticalLine = otherLeft; break; } if (Math.abs(draggingRight - otherRight) < snapThreshold) { snappedX = otherRight - element.width; showVerticalLine = otherRight; break; } if (Math.abs(draggingLeft - otherRight) < snapThreshold) { snappedX = otherRight; showVerticalLine = otherRight; break; } if (Math.abs(draggingRight - otherLeft) < snapThreshold) { snappedX = otherLeft - element.width; showVerticalLine = otherLeft; break; } }
			for (const other of allElements) { if (other.id === element.id) continue; const otherCenterV = other.y + other.height / 2; const otherTop = other.y; const otherBottom = other.y + other.height; if (Math.abs(draggingCenterV - otherCenterV) < snapThreshold) { snappedY = otherCenterV - element.height / 2; showHorizontalLine = otherCenterV; break; } if (Math.abs(draggingTop - otherTop) < snapThreshold) { snappedY = otherTop; showHorizontalLine = otherTop; break; } if (Math.abs(draggingBottom - otherBottom) < snapThreshold) { snappedY = otherBottom - element.height; showHorizontalLine = otherBottom; break; } if (Math.abs(draggingTop - otherBottom) < snapThreshold) { snappedY = otherBottom; showHorizontalLine = otherBottom; break; } if (Math.abs(draggingBottom - otherTop) < snapThreshold) { snappedY = otherTop - element.height; showHorizontalLine = otherTop; break; } }
			...
			onShowSnapLine({ type: 'vertical', position: showVerticalLine }); onShowSnapLine({ type: 'horizontal', position: showHorizontalLine });
		...
		function onDragEnd(e: MouseEvent) {
            ...
            if (didMove) { onUpdate(element.id, { x: currentX, y: currentY }, true); } // isFinalChange = true
  ...
  // --- LÓGICA DE REDIMENSIONAMIENTO ---
	function onResizeStart(e: MouseEvent) {
		...
		function onResizeMove(e: MouseEvent) {
			...
			if (element.type === 'image') { const aspectRatio = startHeight > 0 ? startWidth / startHeight : 0; if (aspectRatio > 0) { if (Math.abs(e.clientX - startX) > Math.abs(e.clientY - startY)) { newHeight = newWidth / aspectRatio; } else { newWidth = newHeight * aspectRatio; newWidth = Math.max(50, newWidth); newHeight = newWidth / aspectRatio; } } else { newHeight = Math.max(30, newHeight); } } else { newHeight = Math.max(30, newHeight); }
			onUpdate(element.id, { width: newWidth, height: newHeight }, false); // isFinalChange = false
		}
		function onResizeEnd() { window.removeEventListener('mousemove', onResizeMove); window.removeEventListener('mouseup', onResizeEnd); if (didResize) onUpdate(element.id, { width: newWidth, height: newHeight }, true); } // isFinalChange = true
  ...
  function handleInput(e: Event) {
    ...
    if (element.content !== target.innerHTML) { // Usar innerHTML para listas
        onUpdate(element.id, { content: target.innerHTML }, false); // Cambio intermedio
    }
  }
  function handleBlur() {
      if (isEditing) {
  ...
  function stopPropagation(e: MouseEvent | FocusEvent) { e.stopPropagation(); }
</script>
...
<style>
    ...
    .draggable-wrapper.editing { border: 1px solid var(--primary-color, #A084E8); cursor: default; box-shadow: none; }
    ...
    .text-content[contenteditable="false"] { cursor: grab; pointer-events: none; }
