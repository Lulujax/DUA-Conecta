<script lang="ts">
	...
	function stopToolbarClick(event: Event) { event.stopPropagation(); }
...
<div class="text-toolbar" role="toolbar" aria-label="Herramientas de texto" tabindex="0" onclick={stopToolbarClick} onmousedown={stopToolbarClick} onkeydown={stopToolbarClick}>
	...
	<div class="font-size-control">
		<button class="icon-button" onclick={() => editorStore.changeFontSize(-1)} title="Reducir tamaño" aria-label="Reducir tamaño">
		...
		<button class="icon-button" onclick={() => editorStore.changeFontSize(1)} title="Aumentar tamaño" aria-label="Aumentar tamaño">
	...
	<label class="icon-button color-label" title="Color de texto">
		...
		<input type="color" class="color-picker-hidden" aria-label="Color de texto" onmousedown={saveSelection} oninput={(e) => toggleFormat('foreColor', e.currentTarget.value)}/>
		<div class="color-indicator" style:background-color={'#000'}></div>
	</label>

    <label class="icon-button color-label" title="Resaltador (Fondo)">
		...
		<input type="color" class="color-picker-hidden" aria-label="Resaltador (Fondo)" onmousedown={saveSelection} oninput={(e) => toggleFormat('hiliteColor', e.currentTarget.value)}/>
		<div class="color-indicator" style:background-color={'yellow'}></div>
	</label>

	<button class="icon-button" class:active={isBold} onmousedown={(e)=>{e.preventDefault(); saveSelection();}} onclick={() => toggleFormat('bold')} title="Negrita (Ctrl+B)" aria-label="Negrita (Ctrl+B)"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M15.6 10.79c.97-.67 1.65-1.77 1.65-2.79 0-2.26-1.75-4-4.25-4H7v14h7.04c2.1 0 3.71-1.7 3.71-3.79 0-1.52-.86-2.82-2.15-3.42zM10 6.5h3c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5h-3v-3zm3.5 9H10v-3h3.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5z"></path></svg></button>
	<button class="icon-button" class:active={isItalic} onmousedown={(e)=>{e.preventDefault(); saveSelection();}} onclick={() => toggleFormat('italic')} title="Cursiva (Ctrl+I)" aria-label="Cursiva (Ctrl+I)"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M10 4v3h2.21l-3.42 8H6v3h8v-3h-2.21l3.42-8H18V4z"></path></svg></button>
	<button class="icon-button" class:active={isUnderlined} onmousedown={(e)=>{e.preventDefault(); saveSelection();}} onclick={() => toggleFormat('underline')} title="Subrayado (Ctrl+U)" aria-label="Subrayado (Ctrl+U)"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M12 17c3.31 0 6-2.69 6-6V3h-2.5v8c0 1.93-1.57 3.5-3.5 3.5S8.5 12.93 8.5 11V3H6v8c0 3.31 2.69 6 6 6zm-7 2v2h14v-2H5z"></path></svg></button>

	<div class="toolbar-separator"></div>

	<button class="icon-button" onmousedown={(e)=>{e.preventDefault(); saveSelection();}} onclick={() => formatList('insertUnorderedList')} title="Lista con viñetas" aria-label="Lista con viñetas"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M4 4h2v2H4zm0 6h2v2H4zm0 6h2v2H4zm4 1h14v2H8zm0-6h14v2H8zm0-6h14v2H8z"></path></svg></button>
	<button class="icon-button" onmousedown={(e)=>{e.preventDefault(); saveSelection();}} onclick={() => formatList('insertOrderedList')} title="Lista numerada" aria-label="Lista numerada"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M2 17h2v.5H3v1h1v.5H2v1h3v-4H2v1zm1-9h1V4H2v1h1v3zm-1 3h1.8L2 13.1v.9h3v-1H3.2L5 11.9V11H2v1zm5-6v2h14V5H7zm0 14h14v-2H7v2zm0-6h14v-2H7v2z"></path></svg></button>

	<div class="toolbar-separator"></div>

	<button class="icon-button" class:active={editorStore.selectedElement?.textAlign === 'left'} onclick={() => editorStore.changeTextAlign('left')} title="Alinear Izquierda" aria-label="Alinear Izquierda"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M15 15H3v2h12v-2zm0-8H3v2h12V7zM3 13h18v-2H3v2zm0 8h18v-2H3v2zM3 3v2h18V3H3z"></path></svg></button>
	<button class="icon-button" class:active={editorStore.selectedElement?.textAlign === 'center'} onclick={() => editorStore.changeTextAlign('center')} title="Centrar" aria-label="Centrar"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M7 15v2h10v-2H7zm-4 6h18v-2H3v2zm0-8h18v-2H3v2zm4-6v2h10V7H7zM3 3v2h18V3H3z"></path></svg></button>
	<button class="icon-button" class:active={editorStore.selectedElement?.textAlign === 'right'} onclick={() => editorStore.changeTextAlign('right')} title="Alinear Derecha" aria-label="Alinear Derecha"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M3 21h18v-2H3v2zm6-4h12v-2H9v2zm-6-4h18v-2H3v2zm6-4h12V7H9v2zM3 3v2h18V3H3z"></path></svg></button>
    <button class="icon-button" class:active={editorStore.selectedElement?.textAlign === 'justify'} onclick={() => editorStore.changeTextAlign('justify')} title="Justificar" aria-label="Justificar"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M3 21h18v-2H3v2zm0-4h18v-2H3v2zm0-4h18V7H3v2zm0-6v2h18V3H3z"></path></svg></button>
</div>
