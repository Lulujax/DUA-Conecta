<script lang="ts">
	...
	import Draggable from '$lib/editor/Draggable.svelte'; // ¡CORRECCIÓN DE LA RUTA AQUÍ!
	...
	let initialElements: Array<any> = [
        ...
        { id: 2, type: 'text', content: 'Fecha:', x: 350, y: 40, width: 50, height: 
			20, fontSize: 12, color: '#000000', isBold: true, textAlign: 'left', fontFamily: 'Arial', isItalic: false, isUnderlined: false },
        ...
        { id: 103, type: 'text', content: 
			'______', x: 605, y: 40, width: 80, height: 20, fontSize: 12, color: '#000000', textAlign: 'left', fontFamily: 'Arial', isItalic: false, isUnderlined: false},
        ...
        { id: 6, type: 'text', content: 'El monstruo siente muchas emociones, dice que en cada parte de su cuerpo hay una emoción ¿En qué parte del cuerpo sientes la emoción?', x: 
			50, y: 140, width: 600, height: 40, fontSize: 12, color: '#000000', textAlign: 'left', fontFamily: 'Arial', isItalic: false, isUnderlined: false },
        ...
        { id: 11, type: 'text', content: '• AMARILLO - FELICIDAD', x: 70, y: 290, width: 250, height: 18, fontSize: 12, 
			color: '#000000', textAlign: 'left', fontFamily: 'Arial', isItalic: false, isUnderlined: false },
        ...
        { id: 13, type: 'text', content: '¡Gana tu ficha aquí!', x: 380, y: 920, width: 160, height: 20, fontSize: 12, color: 
			'#000000', isBold: true, textAlign: 'right', fontFamily: 'Arial', isItalic: false, isUnderlined: false},
    ...
    // Obtiene una referencia al elemento DOM editable y la posición del cursor
    function getEditableContentRef() {
        if (!browser || selectedElementId === null) return null;
        const draggableElement = document.querySelector(`[data-element-id="${selectedElementId}"] .text-content`);
        if (draggableElement && draggableElement instanceof HTMLElement) {
             const selection = window.getSelection();
             const range = selection?.rangeCount ? selection.getRangeAt(0) : null;
            return { element: draggableElement, selection, range };
        }
        return null;
    }

    // Restaura el foco y el cursor
    async function restoreFocus(savedRange: Range | null) {
        if (!browser || selectedElementId === null) return;
        await tick(); // Esperar el re-renderizado
        
        // Hacemos el re-fetch del elemento ya que pudo ser re-renderizado
        const { element, selection } = getEditableContentRef() || {};

        if (element && selection && savedRange) {
            element.focus();
            selection.removeAllRanges();
            selection.addRange(savedRange);
        } else if (element) {
             // Si no hay rango guardado, solo asegurar el foco
            element.focus();
        }
    }
    
    // Función para guardar el contenido del DOM en el estado antes de un cambio de estilo
    function commitCurrentTextContent(data: any = {}, isFinalChange: boolean = true): Range | null {
        if (!browser || selectedElementId === null || selectedElement?.type !== 'text') {
            // Si no es un elemento de texto seleccionado, simplemente aplicamos la actualización si hay datos
             if(Object.keys(data).length > 0) updateElement(selectedElementId!, { ...data }, isFinalChange);
            return null;
        }

        const { element, range } = getEditableContentRef() || {};
        
        if (element && element.innerHTML !== selectedElement.content) {
            // Guardamos el contenido del DOM en el estado, y esto guarda el primer paso en el historial
            updateElement(selectedElementId, { content: element.innerHTML, ...data }, isFinalChange);
            return range; // Devolvemos el rango para restaurar el cursor
        }
         
         // Si no hubo cambio de contenido, aplicamos la actualización solo de estilo (si existe)
    ...
    const initialElementsString = JSON.stringify(elements); 

	// FIX BUG: Inicializamos el historial con el estado real.
	let history: Array<string> = $state([initialElementsString]);
	let historyIndex = $state(0);
	let applyingHistory = $state(false);
	let nextZIndex = $state(elements.length + 1);
    
    // Función auxiliar para quitar la propiedad 'z' (Z-index) del estado y ordenar por ID
    function stripZIndex(elements: Array<any>) {
        return elements.map(el => {
            const { z, ...rest } = el; // Usamos destructuring para excluir 'z'
            return rest;
        }).sort((a, b) => a.id - b.id); // Ordenamos por ID para una comparación consistente
    }

	function saveStateToHistory() {
		if (applyingHistory) return;
        
        // 1. Crear versión del estado actual sin el Z-index para la comparación
        const strippedCurrentElements = stripZIndex(elements);
        const strippedCurrentStateString = JSON.stringify(strippedCurrentElements);

        let strippedLastSavedStateString: string | undefined;
        try {
            // 2. Crear versión del estado anterior sin el Z-index
            const lastSavedElements = JSON.parse(history[historyIndex]);
            const strippedLastSavedElements = stripZIndex(lastSavedElements);
            strippedLastSavedStateString = JSON.stringify(strippedLastSavedElements);
        } catch {}

        // FIX BUG: Si los estados funcionales son iguales, ignorar el cambio (evita guardar la selección/z-index).
        if (strippedCurrentStateString === strippedLastSavedStateString) return;

        // Si hay un cambio funcional, guardar el estado COMPLETO.
        const fullStateString = JSON.stringify(elements);
		const nextHistory = history.slice(0, historyIndex + 1);
		nextHistory.push(fullStateString);
		history = nextHistory;
	...
	async function loadStateFromHistory(index: number) {
        ...
        selectedElementId = null; // Deseleccionar al revertir evita interacciones extrañas
        hasUnsavedChanges = true;
        await tick();
	...
	function undo() { 
        if (!canUndo) return;
        // Forzamos un commit del estado actual del DOM antes de deshacer.
        if (selectedElement?.type === 'text') commitCurrentTextContent({}, true);
	...
	function updateElement(id: number, data: any, isFinalChange: boolean = true) {
		...
		if (index !== -1) {
			 if (elements[index].type === 'image' && data.width !== undefined && data.height === undefined) { const oldWidth = elements[index].width;
				const oldHeight = elements[index].height; if (oldWidth > 0 && oldHeight > 0) data.height = data.width / (oldWidth / oldHeight);
			}
			 else if (elements[index].type === 'image' && data.height !== undefined && data.width === undefined) { const oldWidth = elements[index].width;
	...
	function selectElement(id: number, e: MouseEvent | KeyboardEvent) { 
        ...
        // Si el elemento ya está seleccionado, no hace falta actualizar el z-index ni el id.
        ...
        if (index !== -1) {
            // Se actualiza el Z-index. Este cambio visual NO genera un paso en el historial.
            elements[index].z = nextZIndex++;
        }
    }
	...
	function toggleStyle(styleProp: 'isBold' | 'isItalic' | 'isUnderlined') { 
        if (selectedElement?.type === 'text' && selectedElementId !== null) {
            // Guardamos el contenido del DOM y aplicamos el nuevo estilo en un solo paso
            const savedRange = commitCurrentTextContent({ [styleProp]: !selectedElement[styleProp] }, true);
            restoreFocus(savedRange);
        }
	}
	function changeTextAlign(align: 'left' | 'center' | 'right' | 'justify') { 
        if (selectedElement?.type === 'text' && selectedElementId !== null) {
            const savedRange = commitCurrentTextContent({ textAlign: align }, true);
            restoreFocus(savedRange);
        }
	}
	function changeFontSize(delta: number) { 
        if (selectedElement?.type === 'text' && selectedElementId !== null) { 
            ...
            const savedRange = commitCurrentTextContent({ fontSize: newSize }, true);
            restoreFocus(savedRange);
        } 
	}
    ...
    function formatList(command: 'insertUnorderedList' | 'insertOrderedList') { 
        if (!browser || !selectedElement || selectedElement.type !== 'text' || selectedElementId === null) return;
        
        // 1. Guardar el rango ANTES de cualquier mutación de estado.
        const editableRef = getEditableContentRef();
        const savedRange = editableRef?.range || null; 

        // 2. Forzamos un commit de contenido pendiente, manteniendo el cursor guardado.
        commitCurrentTextContent({}, true);
        
        // 3. Re-obtener el elemento (pudo haber sido re-renderizado en el commit)
        const newEditableRef = getEditableContentRef();
        const newDraggableElement = newEditableRef?.element;
        
        if (newDraggableElement) {
            // 4. Ejecutar el comando del navegador (DOM ONLY). 
            newDraggableElement.focus(); 
            document.execCommand(command, false);
            
            // 5. Pequeño timeout para leer el DOM actualizado y guardarlo como el estado FINAL.
            setTimeout(() => { 
                if (selectedElementId !== null) {
                    updateElement(selectedElementId, { content: newDraggableElement.innerHTML }, true); 
                }
                // 6. Restaurar el foco/cursor.
                restoreFocus(savedRange);
            }, 0);
        } 
	}
...
<main class="editor-layout" onclick={deselect}>
	<aside class="editor-sidebar">
		...
		{#if selectedElement}
			<div class="tool-section">
				<h3>Seleccionado</h3>
				{#if selectedElement.type === 'image'}<p class="prop-label" style="margin-top:0;">Imagen</p><p style="font-size: 0.8rem;
					color: var(--text-light);">Arrastra para mover, usa la esquina para redimensionar.</p>{/if}
				{#if selectedElement.type === 'text'}<p class="prop-label" style="margin-top:0;">Texto</p><p style="font-size: 0.8rem;
					color: var(--text-light);">Edita en la barra superior.</p>{/if}
	...
	<div class="editor-main-area">
		{#if selectedElement?.type === 'text'}
            <div class="text-toolbar" role="toolbar" aria-label="Herramientas de texto" onclick={stopToolbarClick} onmousedown={stopToolbarClick}>
                ...
                <button class="icon-button" onclick={(e) => { stopToolbarClick(e); formatList('insertUnorderedList'); }} title="Lista con viñetas"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M4 4h2v2H4zm0 6h2v2H4zm0 6h2v2H4zm4 1h14v2H8zm0-6h14v2H8zm0-6h14v2H8z"></path></svg></button>
                <button class="icon-button" onclick={(e) => { stopToolbarClick(e); formatList('insertOrderedList'); }} title="Lista numerada"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M2 17h2v.5H3v1h1v.5H2v1h3v-4H2v1zm1-9h1V4H2v1h1v3zm-1 3h1.8L2 13.1v.9h3v-1H3.2L5 11.9V11H2v1zm5-6v2h14V5H7zm0 14h14v-2H7v2zm0-6h14v-2H7v2z"></path></svg></button>
				<div class="toolbar-separator"></div>
...
<style>
    ...
    .font-size-input { width: 35px;
		text-align: center; margin: 0; padding: 0.2rem; border: none; height: 26px; background-color: transparent;
		}
    ...
    .text-toolbar .icon-button:hover:not(:disabled) { background-color: var(--bg-section); color: var(--primary-color);
		border-color: transparent;}
    .text-toolbar .icon-button.active { background-color: var(--primary-color); color: var(--text-on-primary);
		border-color: var(--primary-color);}
    .color-label { position: relative; border: 1px solid var(--border-color); display: flex; flex-direction: column; align-items: center;
		justify-content: center; padding: 2px; box-sizing: border-box;}
    ...
    .color-picker-hidden { position: absolute; top: 0; left: 0;
		width: 100%; height: 100%; opacity: 0; cursor: pointer; }
    .color-indicator { width: 100%; height: 4px; border-radius: 1px;
		margin-top: auto; border: 1px solid rgba(0,0,0,0.1); box-sizing: border-box;}
    .toolbar-separator { width: 1px; height: 18px; background-color: var(--border-color);
		margin: 0 0.3rem; }
    .editor-canvas-area { flex-grow: 1; display: flex; justify-content: center; align-items: flex-start; padding: 2rem;
		padding-top: 70px; overflow: auto; background-color: var(--bg-section); }
    .canvas-container { position: relative; width: 700px; height: 990px; background-color: white;
		box-shadow: 0 10px 30px rgba(0,0,0,0.15); overflow: hidden; border: 1px solid #ccc; flex-shrink: 0;
		}
