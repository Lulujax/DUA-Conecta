<script lang="ts">
	...
	import { browser } from '$app/environment'; // <-- CORREGIDO: browser en minúsculas
	...
	// --- LÓGICA DE HISTORIAL ---
	let history: Array<string> = $state([JSON.stringify(initialElements)]);
	...
	function undo() {
		if (historyIndex > 0) {
			...
			hasUnsavedChanges = true;
			setTimeout(() => { applyingHistory = false; }, 50);
		}
	}
	...
	function updateElement(id: number, data: any, isFinalChange: boolean = true) {
		...
		if (index !== -1) {
			 if (elements[index].type === 'image' && data.width !== undefined && data.height === undefined) {
				 const oldWidth = elements[index].width; const oldHeight = elements[index].height;
				 if (oldWidth > 0 && oldHeight > 0) { data.height = data.width / (oldWidth / oldHeight); }
			 } else if (elements[index].type === 'image' && data.height !== undefined && data.width === undefined) {
				 const oldWidth = elements[index].width; const oldHeight = elements[index].height;
				 if (oldWidth > 0 && oldHeight > 0) { data.width = data.height * (oldWidth / oldHeight); }
			 }

			elements = elements.map((el) =>
				el.id === id ? { ...el, ...data, z: data.z ?? el.z } : el
			);

			if (isFinalChange) saveStateToHistory(); // Solo guardar historial en cambios finales
			// Marcar siempre como cambio pendiente mientras se interactúa o si es final
			if (isFinalChange || !applyingHistory) { // Evita marcar si viene de undo/redo
                hasUnsavedChanges = true;
            }
		}
	}
	...
	beforeNavigate(({ cancel }) => {
		...
		if (!hasUnsavedChanges || !cancel) blockNavigation = false;
	});
    ...
    // Función para prevenir deselección al hacer clic en la barra
	...
	function handleKeyDown(event: KeyboardEvent) {
		...
		if (event.ctrlKey || event.metaKey) {
			if (event.key === 'z') { event.preventDefault(); undo(); }
			 ...
			 if (selectedElementId !== null) { event.preventDefault(); deleteSelected(); }
		}
	}

	$effect(() => {
		if (browser) { // <-- CORREGIDO: browser en minúsculas
...
<main class="editor-layout" onclick={deselect}>
	<aside class="editor-sidebar">
		...
		<div class="tool-section history-controls">
			<button class="icon-button" onclick={undo} disabled={!canUndo} title="Deshacer (Ctrl+Z)">
				<svg viewBox="0 0 24 24"><polyline points="1 4 1 10 7 10"></polyline><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"></path></svg>
			</button>
			<button class="icon-button" onclick={redo} disabled={!canRedo} title="Rehacer (Ctrl+Y)">
			...
			<div class="tool-section">
				...
				{#if selectedElement.type === 'image'}
					...
					<p style="font-size: 0.8rem; color: var(--text-light);">
						Puedes arrastrarla o usar la esquina para redimensionar.
					</p>
				{/if}
				{#if selectedElement.type === 'text'}
					<p class="prop-label" style="margin-top:0;">Texto</p>
					<p style="font-size: 0.8rem; color: var(--text-light);">
						Edita las propiedades en la barra superior.
					</p>
				{/if}
				<button class="btn-secondary btn-delete" onclick={deleteSelected} title="Eliminar (Supr/Borrar)">
					Eliminar Elemento
				</button>
			</div>
		{/if}
	...
	<div class="editor-main-area">
		{#if selectedElement?.type === 'text'}
			<div class="text-toolbar" onclick={stopToolbarClick}> <select
					class="toolbar-select"
					value={selectedElement.fontFamily}
					onchange={(e) => updateElement(selectedElementId!, { fontFamily: e.currentTarget.value }, true)}
				>
				...
				<div class="font-size-control">
					<button class="icon-button" onclick={() => changeFontSize(-1)} title="Reducir tamaño" disabled={selectedElement.fontSize <= 8}>
                        <svg viewBox="0 0 24 24"><path fill="currentColor" d="M19 13H5v-2h14v2z"></path></svg>
                    </button>
					<input
						type="number"
						class="toolbar-input font-size-input"
						min="8" max="120" step="1"
						value={selectedElement.fontSize}
						onchange={updateFontSizeFromInput}
                        onblur={updateFontSizeFromInput}
                        onkeydown={(e) => { if(e.key === 'Enter') { e.preventDefault(); updateFontSizeFromInput(e); (e.target as HTMLElement).blur();} }}
					/>
					<button class="icon-button" onclick={() => changeFontSize(1)} title="Aumentar tamaño" disabled={selectedElement.fontSize >= 120}>
				...
				<label class="icon-button color-label" title="Color de texto">
					<svg viewBox="0 0 24 24"><path fill="currentColor" d="M12 3c-4.97 0-9 4.03-9 9s4.03 9 9 9c.83 0 1.5-.67 1.5-1.5 0-.39-.15-.74-.39-1.01-.23-.26-.38-.61-.38-.99 0-.83.67-1.5 1.5-1.5H16c2.76 0 5-2.24 5-5 0-4.42-4.03-8-9-8zm-5.5 9c-.83 0-1.5-.67-1.5-1.5S5.67 9 6.5 9 8 9.67 8 10.5 7.33 12 6.5 12zm3-4c-.83 0-1.5-.67-1.5-1.5S8.67 5 9.5 5s1.5.67 1.5 1.5S10.33 8 9.5 8zm5 0c-.83 0-1.5-.67-1.5-1.5S13.67 5 14.5 5s1.5.67 1.5 1.5S15.33 8 14.5 8zm3 4c-.83 0-1.5-.67-1.5-1.5S16.67 9 17.5 9s1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"></path></svg>
				...
				<button class="icon-button" class:active={selectedElement.isBold} onclick={() => toggleStyle('isBold')} title="Negrita (Ctrl+B)">
					<svg viewBox="0 0 24 24"><path fill="currentColor" d="M15.6 10.79c.97-.67 1.65-1.77 1.65-2.79 0-2.26-1.75-4-4.25-4H7v14h7.04c2.1 0 3.71-1.7 3.71-3.79 0-1.52-.86-2.82-2.15-3.42zM10 6.5h3c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5h-3v-3zm3.5 9H10v-3h3.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5z"></path></svg>
				</button>
				<button class="icon-button" class:active={selectedElement.isItalic} onclick={() => toggleStyle('isItalic')} title="Cursiva (Ctrl+I)">
					<svg viewBox="0 0 24 24"><path fill="currentColor" d="M10 4v3h2.21l-3.42 8H6v3h8v-3h-2.21l3.42-8H18V4z"></path></svg>
				</button>
				<button class="icon-button" class:active={selectedElement.isUnderlined} onclick={() => toggleStyle('isUnderlined')} title="Subrayado (Ctrl+U)">
					<svg viewBox="0 0 24 24"><path fill="currentColor" d="M12 17c3.31 0 6-2.69 6-6V3h-2.5v8c0 1.93-1.57 3.5-3.5 3.5S8.5 12.93 8.5 11V3H6v8c0 3.31 2.69 6 6 6zm-7 2v2h14v-2H5z"></path></svg>
				</button>

				<div class="toolbar-separator"></div>

				<button class="icon-button" class:active={selectedElement.textAlign === 'left'} onclick={() => changeTextAlign('left')} title="Alinear Izquierda">
					<svg viewBox="0 0 24 24"><path fill="currentColor" d="M15 15H3v2h12v-2zm0-8H3v2h12V7zM3 13h18v-2H3v2zm0 8h18v-2H3v2zM3 3v2h18V3H3z"></path></svg>
				</button>
				<button class="icon-button" class:active={selectedElement.textAlign === 'center'} onclick={() => changeTextAlign('center')} title="Centrar">
					<svg viewBox="0 0 24 24"><path fill="currentColor" d="M7 15v2h10v-2H7zm-4 6h18v-2H3v2zm0-8h18v-2H3v2zm4-6v2h10V7H7zM3 3v2h18V3H3z"></path></svg>
				</button>
				<button class="icon-button" class:active={selectedElement.textAlign === 'right'} onclick={() => changeTextAlign('right')} title="Alinear Derecha">
					<svg viewBox="0 0 24 24"><path fill="currentColor" d="M3 21h18v-2H3v2zm6-4h12v-2H9v2zm-6-4h18v-2H3v2zm6-4h12V7H9v2zM3 3v2h18V3H3z"></path></svg>
				</button>
                <button class="icon-button" class:active={selectedElement.textAlign === 'justify'} onclick={() => changeTextAlign('justify')} title="Justificar">
					<svg viewBox="0 0 24 24"><path fill="currentColor" d="M3 21h18v-2H3v2zm0-4h18v-2H3v2zm0-4h18v-2H3v2zm0-4h18V7H3v2zm0-6v2h18V3H3z"></path></svg>
				</button>
			</div>
		{/if}
...
<style>
    ...
    .toolbar-button { /* Base para B/I/U/Align - Usado como clase ahora */
        display: flex;
     ...
     /* Hereda de .icon-button ahora */
    .toolbar-button.icon-only svg { width: 18px; height: 18px; fill: currentColor; }
    .toolbar-button:hover:not(:disabled) { background-color: var(--bg-section); color: var(--primary-color); }
    .toolbar-button.active { background-color: var(--primary-color); color: var(--text-on-primary); }

    .color-label { /* Hereda de .icon-button */ position: relative; font-weight: bold; border: 1px solid var(--border-color); font-size: 0.8rem; display: flex; flex-direction: column; align-items: center; justify-content: center; width: 28px; height: 28px; background-color: transparent; color: var(--text-light); padding: 2px; box-sizing: border-box;}
