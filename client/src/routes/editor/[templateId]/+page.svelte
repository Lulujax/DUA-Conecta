...
color: var(--text-light);">Edita en la barra superior.</p>{/if}
				{#if selectedElement.type === 'shape'}
					...
					<label style="display:flex;gap:8px;align-items:center;margin-top:6px;">
						Grosor:
						<input type="number" min="1" max="40" value={selectedElement.strokeWidth ||
4} onchange={(e) => updateElement(selectedElementId!, { strokeWidth: parseInt((e.currentTarget as HTMLInputElement).value) || 1 }, true)} />
					</label>
					<label style="display:flex;gap:8px;align-items:center;margin-top:6px;">
						Rotación:
						<input type="range" min="0" max="360" value={selectedElement.rotation ||
0} onchange={(e) => updateElement(selectedElementId!, { rotation: parseInt((e.currentTarget as HTMLInputElement).value) || 0 }, true)} />
						<span style="width:36px;text-align:center">{selectedElement.rotation ||
0}°</span>
					</label>
				{/if}
...
updateFontSizeFromInput(e); (e.target as HTMLElement).blur();} }}/>
				...
				<label class="icon-button color-label" title="Color de texto">
					...
					<div class="color-indicator" style:background-color={selectedElement.color ||
'#000000'}></div>
				</label>
				...
				<button class="icon-button" onclick={(e) => { e.preventDefault();
e.stopPropagation(); formatList('insertUnorderedList'); }} title="Lista con viñetas"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M4 4h2v2H4zm0 6h2v2H4zm0 6h2v2H4zm4 1h14v2H8zm0-6h14v2H8zm0-6h14v2H8z"></path></svg></button>
				<button class="icon-button" class:active={false} onclick={(e) => { e.preventDefault();
e.stopPropagation(); formatList('insertOrderedList'); }} title="Lista numerada"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M2 17h2v.5H3v1h1v.5H2v1h3v-4H2v1zm1-9h1V4H2v1h1v3zm-1 3h1.8L2 13.1v.9h3v-1H3.2L5 11.9V11H2v1zm5-6v2h14V5H7zm0 14h14v-2H7v2zm0-6h14v-2H7v2z"></path></svg></button>
				<div class="toolbar-separator"></div>
		...
		<div class="editor-canvas-area" onclick={deselect}>
			<div class="canvas-container" onmousedown={deselect} bind:this={canvasContainerRef}>
				...
				{/each}
				{#if verticalSnapLine !== null}<div
class="snap-line vertical" style:left="{verticalSnapLine}px"></div>{/if}
