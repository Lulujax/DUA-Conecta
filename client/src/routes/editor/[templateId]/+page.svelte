<script lang="ts">
    ...
    /**
     * @description Envia un evento 'toggle-list' al wrapper del elemento seleccionado.
     * El Draggable escucha ese evento y aplica la conversión a ul/ol sin usar execCommand.
     */
    async function formatList(command: 'insertUnorderedList' | 'insertOrderedList') {
        ...
        if (!wrapper) return;
        // despachar evento custom; el hijo (Draggable) lo procesa y guardará el HTML final
