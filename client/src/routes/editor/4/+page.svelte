<script lang="ts">
	...
	const templateId = '4'; // ID de esta plantilla
    ...
    // --- *** PLANTILLA CORREGIDA: El Dado de las Historias (Fiel) *** ---
    // Estas son las coordenadas y tamaños correctos
    const CUBE_SIZE = 150;
    const ICON_SIZE = 100;
    ...
    // Asegura que los nuevos elementos tengan un z-index alto
	let nextZIndex = $state(BASE_ELEMENTS.reduce((max, el) => Math.max(max, el.z || 0), 0) + 10); 
	...
	async function loadActivity(id: number) {
        ...
        try {
            ...
            nextZIndex = loadedElements.reduce((max, el) => Math.max(max, el.z || 0), 0) + 10;
    ...
    // --- LÓGICA DE ACTUALIZACIÓN CORREGIDA (YA NO BLOQUEA NADA) ---
    ...
    // --- LÓGICA DE SELECCIÓN CORREGIDA (YA NO BLOQUEA NADA) ---
	function selectElement(id: number, e: MouseEvent | KeyboardEvent) { 
        e.stopPropagation();
		selectedElementId = id;
        const index = elements.findIndex((el) => el.id === id);
		if (index !== -1) {
            // Siempre trae el elemento seleccionado al frente
	...
	function deleteSelected() { 
        ...
        // NO permitir borrar los elementos base
        if (selectedElementId >= 1 && selectedElementId <= 37) {
            alert("No puedes eliminar los elementos base de la plantilla.");
            return;
        }
		...
		{#if selectedElement}
			<div class="tool-section">
                ...
                {#if selectedElement.type === 'shape' || selectedElement.type === 'image'}
                    <label style="display:flex;gap:8px;align-items:center;margin-top:6px;">
						Rotación:
						<input type="range" min="0" max="360" value={selectedElement.rotation || 0} oninput={(e) => updateElement(selectedElementId!, { rotation: parseInt((e.currentTarget as HTMLInputElement).value) || 0 }, false)} onchange={(e) => updateElement(selectedElementId!, { rotation: parseInt((e.currentTarget as HTMLInputElement).value) || 0 }, true)} />
